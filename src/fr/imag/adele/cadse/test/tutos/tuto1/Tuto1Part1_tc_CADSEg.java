...
public class Tuto1Part1_tc_CADSEg extends TutoTestCase {
	...
	public void test_data_model() throws Exception {
		...
		shell = new GTCadseShell(CadseGCST.LINK);
		GTCadseFactory.findField(shell, CadseGCST.ITEM_at_NAME_).typeText("hasComp");
		GTCadseFactory.findField(shell, CadseGCST.LINK_lt_DESTINATION).browser("WebAppModel", CadseDefinitionManager.DATA_MODEL, "WebComponent");
		GTCadseFactory.findField(shell, CadseGCST.LINK_at_PART_).check(true);
		GTCadseFactory.findField(shell, CadseGCST.LINK_at_REQUIRE_).check(true);
		...
		GTCadseFactory.findField(shell, CadseGCST.LINK_lt_DESTINATION).browser("WebAppModel", CadseDefinitionManager.DATA_MODEL, "Library");
		GTCadseFactory.findField(shell, CadseGCST.LINK_at_AGGREGATION_).check(false);
