...
public class Tuto2Part4_tc_CADSEg extends TutoTestCase {
	...
	public void test_build_model_definition() throws Exception {
		...
		GTCadseFactory.findCadseWorkbenchPart(propertiesView).findField(CadseGCST.LINK_TYPE_at_COMPOSITION_).check(true);
	...
	public void test_copy_composer() throws Exception {
		...
		GTCadseFactory.findCadseWorkbenchPart(shell).findField(CadseGCST.ITEM_at_NAME_).typeText("GenWarComposer");
		GTCadseFactory.findCadseWorkbenchPart(shell).findField(CopyComposerCST.COPY_INTO_FOLDER_COMPOSER_at_TARGET_FOLDER_).typeText("WEB-INF/classes");
		GTCadseFactory.findCadseWorkbenchPart(shell).findField(CadseGCST.COMPOSER_at_TYPES_).add("ref-classes");
		GTCadseFactory.findCadseWorkbenchPart(shell).findField(CadseGCST.RUNTIME_ITEM_at_EXTENDS_CLASS_).check(true);
